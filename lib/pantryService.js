...
export async function getPantryItems() {
  ...
  return querySnapshot.docs.map((doc) => ({ itemId: doc.id, ...doc.data() }));
...
/* UPDATE PANTRY ITEM */
export async function updatePantryItem(itemId, newData) {
	try {
		const itemRef = doc(firestore, PANTRY_COLLECTION, itemId);
